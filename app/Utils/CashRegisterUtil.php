...
class CashRegisterUtil extends Util
{
    ...
    public function countOpenedRegister()
    {
        ...
        $count =  CashRegister::where('admin_id', $admin_id)
            ->where('status', 'open')
            ->count();
        return $count;
    }
     ...
     * @return obj
     */
    public function getCurrentCashRegister($admin_id)
    {
        $register =  CashRegister::where('admin_id', $admin_id)
            ->where('status', 'open')
            ->first();
     ...
     * @return obj
     */
    public function getCurrentCashRegisterOrCreate($admin_id)
    {
        $register =  CashRegister::where('admin_id', $admin_id)
            ->where('status', 'open')
            ->first();
     ...
     * @return boolean
     */
    public function addPayments($transaction, $payment, $type = 'credit', $admin_id = null, $transaction_payment_id = null)
    {
        if (empty($admin_id)) {
            $admin_id = auth()->user()->id;
        }
        $register =  $this->getCurrentCashRegisterOrCreate($admin_id);
    ...
    public function updateAddStockAndExpensePayments($transaction, $payment_data, $request)
    {
        ...
        $cash_register_transaction = $this->updateCashRegisterTransaction($cr_transaction->id, $register, $payment_data['amount'], $transaction->type, 'debit', $refercene_register->admin_id, '', $referenced_id);

        $refercene_transaction = CashRegisterTransaction::where('id', $referenced_id)->first();


        $admin_id = $register->admin_id;
        $cash_register_transaction_out = $this->updateCashRegisterTransaction($referenced_id, $refercene_register, $payment_data['amount'], 'cash_out', 'credit', $admin_id, '', $cr_transaction->id);
    ...
    public function updateSellPaymentsBasedOnPaymentDate($transaction, $payments)
    {
        ...
        $opened_register =  CashRegister::where('admin_id', $transaction->created_by)
            ->where('status', 'open')
            ->first();
    ...
    public function updateSellPayments($transaction, $payments)
    {
        $admin_id = auth()->user()->id;
        $transaction = Transaction::find($transaction->id);
        $register =  CashRegister::where('admin_id', $transaction->created_by)
            ->where('status', 'open')
            ->first();
        if ($transaction->status == 'final') {
            $prev_payments = CashRegisterTransaction::where('transaction_id', $transaction->id)
