...
//                    $final_total = preg_match('/\.\d*[1-9]+/', (string)$final_total) ? $final_total : number_format($final_total,  2, '.', ',');
    ...
    public function getAdminReport(Request $request)
    {
        ...
        if (!empty($admin_id)) {
            $sale_query->where('transactions.created_by', $admin_id);
        }
        ...
        if (!empty($admin_id)) {
            $add_stock_query->where('transactions.created_by', $admin_id);
        }
        ...
        if (!empty($admin_id)) {
            $quotation_query->where('transactions.created_by', $admin_id);
        }
        ...
        if (!empty($admin_id)) {
            $return_query->where('transactions.created_by', $admin_id);
        }
        ...
        if (!empty($admin_id)) {
            $expense_query->where('transactions.expense_beneficiary_id', $admin_id);
        }
    ...
    public function getSalesPerEmployeeReport(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            if (!auth()->user()->can('reports.sales_per_employee.view')) {
                $employee = Employee::where('admin_id', Auth::id())->first();
                if (!empty($employee)) {
