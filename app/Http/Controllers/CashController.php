...
class CashController extends Controller
{
    ...
    public function index()
    {
        ...
        if (!auth()->user()->can('cash.view_details.view')) {
            $query->where('admin_id', Auth::user()->id);
        }
        ...
        if (!empty(request()->admin_id)) {
            $query->where('cash_registers.admin_id', request()->admin_id);
        }
        ...
        ->whereIn('transaction_id', function ($query) use ($cash_register) {
            $query->select('id')
                ...
                OrWhere(function ($query) use ($cash_register) {
                    $query->whereRaw('created_at <> updated_at');
                    $query->WhereRaw('updated_at >= (created_at + INTERVAL 1 MINUTE)');
                    $query->where('created_by', '!=', $cash_register->admin_id)
        ...
        try {
            ...
            if (!empty($request->source_id)) {
                if ($request->source_type == 'user') {
                    ...
                    $cash_register_transaction_out = $this->cashRegisterUtil->createCashRegisterTransaction($register, $amount, 'cash_out', 'credit', $admin_id, $request->notes, $cash_register_transaction->id);
    ...
    public function saveAddCashOut(Request $request)
    {
        try {
            ...
            if (!empty($request->source_id)) {
                if ($request->source_type == 'user') {
                    $register = $this->cashRegisterUtil->getCurrentCashRegisterOrCreate($request->source_id);
                    $cash_register_transaction_in = $this->cashRegisterUtil->createCashRegisterTransaction($register, $amount, 'cash_in', 'debit', $admin_id, $request->notes, $cash_register_transaction->id);
    ...
    public function saveAddClosingCash(Request $request)
    {
        try {
            ...
            if ($request->submit == 'adjustment') {
                ...
                $data['admin_id'] = $register->admin_id;
